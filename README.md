...
* We proviode three .edp here:
  ...
  - `ReferNS_transient_parallel.edp` is designed for performing finite-element simulations (transient) with various shapes of domains on a reference domain (paralleled with PETSc). This step generate the geometry-informed snapshots for ROM. (suggest to run)
